...
export const ViewerCanvas = forwardRef<ViewerCanvasHandle, ViewerCanvasProps>(({ 
    onEntitySelect, onLockChange, pvsEnabled, showPointEntities, showBrushEntities, 
    showBrushWireframes, showAxes, aaaTriggerOpacity, entityConnectionsMode, 
    textureFiltering, lightmapFiltering, selectedEntity, onProgress 
}, ref) => {
    ...
    useImperativeHandle(ref, () => ({
        loadMap: async (bspFile: File, wadFiles: File[], fgdFiles: File[]) => {
            ...
            try {
                ...
                viewerRef.current.setEntitiesVisible(showPointEntities);
    ...
    useEffect(() => { viewerRef.current?.setPvsEnabled(pvsEnabled); }, [pvsEnabled]);
    useEffect(() => { viewerRef.current?.setEntitiesVisible(showPointEntities); }, [showPointEntities]);
    useEffect(() => { viewerRef.current?.setBrushEntitiesVisible(showBrushEntities); }, [showBrushEntities]);
    useEffect(() => { viewerRef.current?.setBrushWireframesVisible(showBrushWireframes); }, [showBrushWireframes]);
    useEffect(() => { viewerRef.current?.setAxesVisible(showAxes); }, [showAxes]);
    useEffect(() => { viewerRef.current?.setAaaTriggerOpacity(aaaTriggerOpacity); }, [aaaTriggerOpacity]);
    useEffect(() => { viewerRef.current?.setEntityConnectionsMode(entityConnectionsMode); }, [entityConnectionsMode]);
    useEffect(() => { viewerRef.current?.setTextureFiltering(textureFiltering); }, [textureFiltering]);
    useEffect(() => { viewerRef.current?.setLightmapFiltering(lightmapFiltering); }, [lightmapFiltering]);

    useEffect(() => {
        if (viewerRef.current) {
            viewerRef.current.highlightEntity(selectedEntity);
            viewerRef.current.setEntityConnectionsMode(entityConnectionsMode);
        }
